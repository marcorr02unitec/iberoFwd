import { Controller, Post, Get, Body, Res, HttpStatus, Put, Delete, Param } from '@nestjs/common';
...
export class ClusterController {
  ...
  @Put("/:cluster")
  async updateCluster(@Res() res, @Body() clusterObject: any,@Param ('cluster')clus): Promise<any> {
    try {
      let cluster: any = await this.clusterService.updateCluster(clus,clusterObject);
      if (!cluster.error) {
        return res.status(HttpStatus.OK).json({
  ...
  @Delete("/:cluster")
  async delteCluster(@Res() res,@Param ('cluster')clus): Promise<any> {
      try {
          let cluster: Array<any> = await this.clusterService.deleteCluster(clus);
          return res.status(HttpStatus.OK).json({
              message: 'success',
              data: cluster,
              statusCode: 200
